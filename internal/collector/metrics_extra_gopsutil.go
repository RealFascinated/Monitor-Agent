...
func coreMetricsFromGopsutil(cores []cpu.CoreUsage) []ingest.CPUCoreMetric {
	if len(cores) == 0 {
		return nil
	}
...
func temperatureMetricsFromGopsutil(readings []thermal.TemperatureReading) []ingest.TemperatureMetric {
	if len(readings) == 0 {
		return nil
	}
